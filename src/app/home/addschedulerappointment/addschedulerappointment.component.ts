...
export class AddschedulerappointmentComponent {
  ...
  updateDetails() {
    this.appointmentData.appDate = moment(this.details.appDate).format('YYYY-MM-DD hh:mm a');
    this.appointmentData.endTime = moment(this.details.endTime).format('YYYY-MM-DD hh:mm a');
    this.appointmentData.startTime = moment(this.details.startTime).format('YYYY-MM-DD hh:mm a');
    this.patientName = this.details.mstUsersByUserPatientTypeId.firstName + ' ' + this.details.mstUsersByUserPatientTypeId.lastName;
  ...
  updateStartDate(event: any) {
    ...
    this.appointmentData.endTime = moment(date).add(1, 'hours').format('YYYY-MM-DD hh:mm a');
  }

  updateEndDate(event: any) {
    const date = moment(event.target.value).format('YYYY-MM-DD hh:mm a');
    this.appointmentData.startTime = moment(date).subtract(1, 'hours').format('YYYY-MM-DD hh:mm a');
  }
  searchPatient() {
    this.appService.warning('', 'You can search for an existing patient by typing their name in the text field.');
  }
  addPatient() {
    if (this.appointmentType === 'obpatient') {
  ...
  submitAppointment() {
    this.appointmentData.startTime = moment(this.appointmentData.startTime).format('MM-DD-YYYY HH:MM');
    this.appointmentData.endTime = moment(this.appointmentData.endTime).format('MM-DD-YYYY HH:MM');
    this.appointmentData.appDate = moment(this.appointmentData.startTime).format('MM-DD-YYYY HH:MM');
    let url: any;
    if (this.addorupadte === 'update') {
    ...
    this.homeService.submitPatientDetails('patient/addPatient', this.appointmentData).subscribe((data) => {
      console.log(data);
      // this.appService.hideLoader();
      if (data) {
        if (this.addorupadte === 'update') {
          this.appService.success('Success', 'Appointment Update Successfully.');
        } else {
